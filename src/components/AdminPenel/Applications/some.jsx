...
const Applications = () => {
  ...
  const [toggleEnquiries, setToggleEnquiries] = useState(
    user.desig === "Receptionist" ? true : false
  );
  const [applicationsLoading, setApplicationsLoading] = useState(true);

  const refToChangeHandler = async (id, name) => {
    try {
      await axios.put(`https://edifygroup.herokuapp.com/api/apply/${id}`, {
        conStatus: "Refered",
        refTo: name,
      });
    } catch (error) {
    ...
    try {
      const fetchAllApplications = async () => {
        const res = await axios.get(
          "https://edifygroup.herokuapp.com/api/apply/"
        );
  ...
  useEffect(() => {
    try {
      const fetchAllCampusApplications = async () => {
        const res = await axios.get(
          "https://edifygroup.herokuapp.com/api/campusApply/"
        );
  ...
  const deleteApplication = async (id) => {
    try {
      if (window.confirm("Are You Sure To Delete This Review")) {
        const deletedApplication = await axios.delete(
          `https://edifygroup.herokuapp.com/api/${
            toggleEnquiries === true ? "campusApply" : "apply"
          }/${id}`
        );
        deletedApplication &&
      ...
      toast.error(
        "Something Went Wronge While Deleting This Application!" + error
      );
    }
  };
            ...
            <div className="modal-body">
              <form>
                <div className="form-group">
                  <h3 for="recipient-name" className="col-form-label">
                    Date.Now()
                  </h3>
                  <div className="chat-text"><p >Lorem ipsum dolor sit amet consectetur adipisicing elit. Vel molestias commodi hic, eum architecto et perferendis quam enim nesciunt vitae maxime? Id rem expedita odio sint ad harum non cumque.</p></div>
                            ...
                            <tbody>
                                {
                                    applicationsLoading ?
                                        ...
                                        allApplications.filter((v) => {
                                            if (filterTerm === "") {
                                                if (user.username === v.refTo) {
                                                    return v
                                                } else if (user.desig !== "Counselor") {
                                                    return v
                                                }
                                            } else if (
                                            ...
                                            return (
                                                <tr key={i} className='EachDataRow'>
                                                    ...
                                                    <td className='actionsIcons'><Link to={`apply/${v._id}`}><i className="fa-solid fa-rectangle-list"></i></Link> {((user.desig === "Admin") || (user.desig === "Developer")) && <i onClick={() => deleteApplication(v._id)} className="fa-solid fa-trash"></i>}</td>
                                                </tr>
                                            )
                                        })
                                }
                                    ...
                                    allCampusApplications.filter((v) => {
                                        ...
                                        return (
                                            <tr key={i} className='EachDataRow'>
                                                ...
                                                <td>{v.refTo}</td>
                                                <td>{v.hereAbout}</td>
                                                <td style={{padding:"4px"}} className='actionsIcons'><i class="fa-solid fa-comment"></i>{((user.desig === "Admin") || (user.desig === "Developer") || (user.desig === "Receptionist")) && <Link to="/admin/campusEnquiryForm" state={v} ><i className="fa-solid updateIcon fa-pen-to-square"></i></Link>} {((user.desig === "Admin") || (user.desig === "Developer")) && <i onClick={() => deleteApplication(v._id)} className="fa-solid fa-trash"></i>}</td>
                                            </tr>
                                        )
                                    })
                                }
...
=======
                <div className="form-group">
                  ...
                  ></textarea> <div className="modal-footer">
...
>>>>>>> 936272b7a5e4d3b20a54dc35033914a41d6c7fb0
            </div>
        ...
        <div style={{ display: "flex", alignItems: "center" }}>
          {user.desig === "Receptionist" ? (
            ...
            <div className="toggleEnquiryTypeBtn">
              <div
                onClick={() => setToggleEnquiries(!toggleEnquiries)}
                className="toggleBtnOuterDiv"
              >
                <div
                  style={{ left: toggleEnquiries !== true ? "0%" : "50%" }}
                  className="toggleNode"
                >
                  {toggleEnquiries !== true ? "W" : "C"}
                </div>
              </div>
            </div>
          )}
          <h3 className="hfont" data-aos="fade-up" data-aos-duration="1200">
            Enquiries
          </h3>
        </div>
        <DownloadTableExcel
      ...
      <div
        className="enquiriesTable ApplicationsTable"
        data-aos="fade-up"
                ...
                allApplications
                  .filter((v) => {
                    if (filterTerm === "") {
                      ...
                      dateFormating(new Date(v.createdAt)).includes(
                        filterTerm
                      ) ||
                      v.name.toLowerCase().includes(filterTerm.toLowerCase()) ||
                      v.edu
                      ...
                      <tr key={i} className="EachDataRow">
                        ...
                        <td className="text-center">
                          {!v.refTo ? (
                            ...
                            >
                              <option value="">Select Counselor</option>
                              <option value="Junaid Sohail">
                                Junaid Sohail
                              </option>
                              <option value="Amaima Iftikhar">
                                Amaima Iftikhar
                              </option>
                              <option value="Nasir Raza">Nasir Raza</option>
                              <option value="Syed Nouman Shah">
                                Syed Nouman Shah
                              </option>
                              <option value="Marwa Yasin">Marwa Yasin</option>
                              <option value="Mehreen Tiwana">
                                Mehreen Tiwana
                              </option>
                              <option value="Muhammad Bilal">
                                Muhammad Bilal
                              </option>
                        ...
                        <td className="actionsIcons">
                          ...
                          {user.desig === "Admin" ||
                            (user.desig === "Developer" && (
                              <i
                                onClick={() => deleteApplication(v._id)}
                ...
                .map((v, i) => {
                  return (
                    <tr key={i} className="EachDataRow">
                      ...
                      <td className="text-center">{v.gender}</td>
                      <td
                        className={v.email.trim() === "" ? "text-center" : ""}
                      >
                      ...
                      <td className={v.ref.trim() === "" ? "text-center" : ""}>
                        {v.ref.toUpperCase() === "" ? "-" : titleCase(v.ref)}
                      </td>
                      <td>{v.pCountries.toString()}</td>
                      <td>{v.refTo}</td>
                      <td>{v.hereAbout}</td>
                      <td className="actionsIcons">
                        {(user.desig === "Admin" ||
                          user.desig === "Developer" ||
                          user.desig === "Receptionist") && (
                          <Link to="/admin/campusEnquiryForm" state={v}>
                            <i className="fa-solid updateIcon fa-pen-to-square"></i>
                          </Link>
                        )}{" "}
                        {user.desig === "Admin" ||
                          (user.desig === "Developer" && (
